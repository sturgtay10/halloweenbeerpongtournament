...
(function(){
  ...
  // Always hide bracket on startup until user begins a new tournament
  if (bracket) bracket.classList.add('hidden');
  if (connectors) connectors.classList.add('hidden');
  if (phaseTitle) phaseTitle.classList.add('hidden');

  // Clear any pre-rendered content
  if (bracket) bracket.innerHTML = '';
  if (connectors) connectors.innerHTML = '';
  if (phaseTitle) phaseTitle.textContent = '';
}
...
function autoScaleBracket() {
  ...
  // Base scale: auto-fit within the wrapper
  let scale = Math.min(1, wrapWidth / (brWidth + 60), wrapHeight / (brHeight + 100));

  // --- Adjust for large brackets ---
  try {
    const teamCount = window.teamCount || 8;
    if (teamCount >= 9 && teamCount <= 10) scale *= 0.92;
    else if (teamCount >= 11 && teamCount <= 13) scale *= 0.88;
    else if (teamCount >= 14) scale *= 0.82;
  } catch (e) {}

  // --- Make the Final Round Bracket noticeably larger ---
  const bracketEl = document.getElementById('bracket');
  const isFinal = bracketEl?.closest('.mode-final');
  if (isFinal) {
    scale *= 1.35; // make grand final ~35% larger
  }

  // --- Keep scale reasonable ---
  scale = Math.max(0.4, Math.min(scale, 1.3));
...
// === SYNC TITLE BATS WITH LIGHTNING FLASH ===
(function() {
  const bats = document.querySelector('.title-bats');
  const lightning = document.querySelector('.lightning');

  if (!bats || !lightning) return;

  // Observe when lightning flashes to boost bats briefly
  const observer = new MutationObserver(() => {
    if (lightning.classList.contains('flash')) {
      bats.classList.add('bats-bright');
      setTimeout(() => bats.classList.remove('bats-bright'), 700);
    }
  });

  observer.observe(lightning, { attributes: true, attributeFilter: ['class'] });
})();

// === RANDOMIZED TITLE BATS (title screen only, non-invasive) ===
(function() {
  try {
    const bats = document.querySelector('.title-bats');
    const titleScreen = document.getElementById('title-screen');
    if (!bats || !titleScreen) return;

    bats.classList.add('rand-anim');
    ...
    function step(ts) {
      ...
      if (ts > flickerAt) {
        const op = rand(0.38, 0.58);
        const br = rand(0.85, 1.28);
        bats.style.opacity = op.toFixed(2);
        bats.style.filter = `brightness(${br.toFixed(2)})`;
        flickerAt = ts + rand(180, 640);
      }
      ...
      driftX += (targetX - driftX) * 0.025;
      driftY += (targetY - driftY) * 0.025;
      bats.style.transform = `translate(-50%, ${driftY.toFixed(2)}px) scale(1.3) rotate(${(driftX*0.4).toFixed(2)}deg)`;
    ...
    const obs = new MutationObserver(() => {
      if (titleScreen.classList.contains('hidden')) stop(); else start();
    });
...
document.getElementById('clear-all').addEventListener('click', () => {
  if (confirm('Are you sure you want to clear everything? This will remove all teams and brackets.')) {
    // 1. Clear saved state
    localStorage.clear();

    // 2. Remove existing bracket visuals
    const bracket = document.getElementById('bracket');
    const connectors = document.getElementById('connectors');
    const phaseTitle = document.getElementById('phase-title');
    ...
    if (connectors) {
      ...
      connectors.classList.add('hidden');
    }
    if (phaseTitle) {
